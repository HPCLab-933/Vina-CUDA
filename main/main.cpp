...
void main_procedure(std::vector<model> &ms, const boost::optional<model> &ref, // m is non-const (FIXME?)
					...
					const flv &weights,
					int cpu, int seed, int verbosity, sz num_modes, fl energy_range,
					tee &log, int search_depth, int thread, std::vector<std::vector<std::string>> ligand_names, int rilc_bfgs)
{
	...
	else
	{
		...
		for (int ligand_count = 0; ligand_count < ligand_num; ligand_count++)
		{
			...
			if (rilc_bfgs == 1)
			{
				tmp_steps = (0.24 * ms[ligand_count].num_movable_atoms() + 0.29 * ms[ligand_count].get_size().ligands[0] - 3.41) * 2;
			}
			else
			{
				tmp_steps = (0.24 * ms[ligand_count].num_movable_atoms() + 0.29 * ms[ligand_count].get_size().ligands[0] - 3.41);
			}
			if (tmp_steps < 1)
				tmp_steps = 1;
	...
	for (int ligand_count = 0; ligand_count < ligand_num; ligand_count++)
	{
		par.mc.ssd_par.bfgs_steps.push_back(unsigned((30 + ms[ligand_count].num_movable_atoms()) / 3));
	}
	...
	for (int ligand_count = 0; ligand_count < ligand_num; ligand_count++)
	{
		...
		{
			std::cout << std::endl
					  << "No results found for ligand " << name_tmp2 << std::endl
					  << std::endl;
			continue;
		}
	...
	*/
	...
	try
	{
		...
		misc.add_options()
			...
			("num_modes", value<int>(&num_modes)->default_value(9), "maximum number of binding modes to generate")("energy_range", value<fl>(&energy_range)->default_value(3.0), "maximum energy difference between the best binding mode and the worst one displayed (kcal/mol)");
		...
		{
			std::cout << "WARNING: The search space volume" << search_box_needed << "> 27000 Angstrom^3 (See FAQ)\n"
					  << std::endl;
		}
		...
		std::cout << "reading time: " << boost_t.elapsed() << std::endl;
		done(verbosity, log);
		...
		main_procedure(ms, ref,
					   ...
					   gd, exhaustiveness,
					   weights,
					   cpu, seed, verbosity, max_modes_sz, energy_range, log, search_depth, thread, ligand_names, rilc_bfgs);
	}
	catch (file_error &e)
	{
